...
@router.post("/admin/db/init", response_model=ActionOut)
def admin_db_init(request: Request, _: AdminDep) -> ActionOut:
    """One-time: tạo schema trên DB trống + stamp alembic_version. Idempotent.

    Cần vì `alembic upgrade head` không chạy được từ máy local (firewall công ty
    chặn wire protocol Postgres tới Neon), nhưng function trên Vercel thì kết nối
    được. ``create_all`` sinh schema khớp models; migration hand-written đã được
    đối chiếu tương đương. Stamp revision để health + migration sau này hoạt động.
    Xoá endpoint này sau khi chạy xong.
    """
    from sqlalchemy import inspect as sa_inspect
    from sqlalchemy import text as sa_text

    import app.db.models  # noqa: F401  — đảm bảo bảng đã đăng ký vào metadata
    from app.db.base import Base

    engine = request.app.state.engine
    Base.metadata.create_all(engine)
    rev = getattr(request.app.state, "alembic_head", None) or "a1b2c3d4e5f6"
    ...
    return ActionOut(ok=True, message=f"tables={tables}; stamped={rev}")
